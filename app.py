...
def chat():
    ...
    if option == "Quiz me about":
        ...
        prompt = f"Return only valid JSON. No plain text whatsoever. Return only valid JSON. Provide only 3 basic, easy, simple and obvious book title words about {user_message}. If the topic is simple, just return 3 of the topic's words themselves in the JSON. Return the response as a JSON object with the following structure: {{\"searchTerms\": [\"term1\", \"term2\", \"term3\"]}}. Ensure the JSON is valid and contains exactly 3 search terms. Return only valid JSON."
    else:
